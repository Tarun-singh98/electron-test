...
export const getAllcostCategoryDataFromTally = async (port: any, companyNameForTally: any) => {
    try {
            ...
            <DESC>
                ...
                <TDL>
                    <TDLMESSAGE>
                        ...
                        <FIELD NAME="Fld04">
                            <SET>if $AllocateRevenue then 1 else 0</SET>
                            <XMLTAG>ALLOCATEREVENUE</XMLTAG>
                        </FIELD>
                        <FIELD NAME="Fld05">
                            <SET>if $AllocateNonRevenue then 1 else 0</SET>
                            <XMLTAG>ALLOCATENONREVENUE</XMLTAG>
                        </FIELD>
                        <FIELD NAME="FldBlank">
                            <SET>""</SET>
                        </FIELD>
...
const converData = (data: any) => {
    try {
        ...
        ALLOCATEREVENUE = (ALLOCATEREVENUE).map((x: any) => parseInt(x))
        ALLOCATENONREVENUE = (ALLOCATENONREVENUE).map((x: any) => parseInt(x))
        return {
            GUID,
            ALTERID,
            NAME,
